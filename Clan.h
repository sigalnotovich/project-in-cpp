...
namespace mtm {
	...
	*/
	class Clan {
		...
		MtmSet<GroupPointer> groups_set;  //the groups of the clan
		MtmSet<std::string> friends_set;  //the friends of the clan
		bool isClanAreTheSame(Clan& other); //need to check
		bool isThereIsGroupsWithTheSameNameInBothClans(Clan& other); //need to check ->and check intersect() and empty() of MtmSet .
																	 //std::ostream& returnSetOrganaizedByPowerFromStrongerToWeakest(std::ostream& os);
		friend bool group_pointer_compare_function_clan(GroupPointer pointer1, GroupPointer pointer2);
		//friend bool group_pointer_compare_function(GroupPointer pointer1, GroupPointer pointer2);
	public:
		/**
		* Constructor
