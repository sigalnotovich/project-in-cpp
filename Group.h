...
namespace mtm {
	...
	*/
	class Group {
		...
		int food;
		int morale;
		int getPower(const Group& group) const; //not needed now
		bool checkIsTwoGroupAreTheSame(Group& other);
		Group& addToLoser(Group& other);
		Group& addToWinner(Group& other, int loser_food);
		void emptyGroup(Group& other);
		void makeTheTradeThisHasMoreFood(Group& other, int trade);
		void makeTheTradeThisHasMoreTools(Group& other, int trade);
		bool checkConditionsForTrade(Group& other);
	public:
		Group() = default; //addad -//fixed 26.1 14:00 //need to remove
		/**
		* Constructor
